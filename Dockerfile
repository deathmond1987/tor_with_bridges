...
HEALTHCHECK --interval=5m --retries=2 \
            CMD if [ -f ${DATA_DIR}/.lock ]; then echo "tor starting...";  else curl --retry 4 --max-time 10 -xs --socks5-hostname 127.0.0.1:${SOCKS_PORT} 'https://check.torproject.org' | tac | grep -qm1 Congratulations; fi || pkill entrypoint.sh && exit 1
